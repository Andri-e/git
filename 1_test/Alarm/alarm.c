...
static UA_StatusCode addCondition_2(UA_Server *server) 
{
...
static void afterWriteCallbackVariable_2(UA_Server *server, const UA_NodeId *sessionId,
                             void *sessionContext, const UA_NodeId *nodeId,
                             void *nodeContext, const UA_NumericRange *range,
                             const UA_DataValue *data) 
{
 ...
 */
static void afterWriteCallbackVariable_3(UA_Server *server,
               ...
               const UA_NumericRange *range, const UA_DataValue *data) 
{
    ...
    retval = UA_Server_setConditionField(server, conditionInstance_1, &value, commentField);
    if(retval != UA_STATUSCODE_GOOD) 
	{
        UA_LOG_ERROR(UA_Log_Stdout, UA_LOGCATEGORY_USERLAND, "Setting Comment Field failed. StatusCode %s",
                     UA_StatusCode_name(retval));
        return;
    }
    ...
    retval = UA_Server_setConditionField(server, conditionInstance_1, &value, retainField);
    if(retval != UA_STATUSCODE_GOOD) 
	{
        UA_LOG_ERROR(UA_Log_Stdout, UA_LOGCATEGORY_USERLAND, "Setting Retain Field failed. StatusCode %s",
                     UA_StatusCode_name(retval));
        return;
    }
    ...
    if (retval != UA_STATUSCODE_GOOD) 
	{
		UA_LOG_WARNING(UA_Log_Stdout, UA_LOGCATEGORY_USERLAND, "Triggering condition event failed. StatusCode %s",
                    UA_StatusCode_name(retval));
		return;
    }
}
...
static UA_StatusCode enteringConfirmedStateCallback(UA_Server *server, const UA_NodeId *condition) 
{
    ...
    if(retval != UA_STATUSCODE_GOOD) 
	{
        UA_LOG_ERROR(UA_Log_Stdout, UA_LOGCATEGORY_USERLAND,"Setting ActiveState/Id Field failed. StatusCode %s",
                     UA_StatusCode_name(retval));
        return retval;
    }
